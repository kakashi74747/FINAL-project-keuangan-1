    ...
    try {
        if ($_POST['action'] == 'tambah' || $_POST['action'] == 'edit') {
            ...
            if ($_POST['action'] == 'tambah') {
                ...
                $id = (int)$_POST['id'];
                // Saat edit, sisa utang tidak diubah, hanya data utama
                $stmt_get_remaining = mysqli_prepare($koneksi, "SELECT remaining_amount FROM debts WHERE id=? AND user_id=?");
                mysqli_stmt_bind_param($stmt_get_remaining, "ii", $id, $user_id);
                mysqli_stmt_execute($stmt_get_remaining);
                $old_debt = mysqli_fetch_assoc(mysqli_stmt_get_result($stmt_get_remaining));
                $remaining_amount = $old_debt['remaining_amount'];
                $status = ($remaining_amount <= 0 && $total_amount > 0) ? 'Lunas' : 'Belum Lunas';

                $stmt = mysqli_prepare($koneksi, "UPDATE debts SET type=?, person_name=?, description=?, total_amount=?, remaining_amount=?, status=?, due_date=? WHERE id=? AND user_id=?");
                mysqli_stmt_bind_param($stmt, "sssddssii", $type, $person_name, $description, $total_amount, $remaining_amount, $status, $due_date, $id, $user_id);
        ...
        elseif ($_POST['action'] == 'bayar') {
            ...
            if ($debt['type'] == 'Utang') { // Kita membayar utang, uang keluar
                ...
                if (!$akun || $akun['current_balance'] < $amount) { throw new Exception("Saldo akun tidak mencukupi."); }
...
<main>
    <div class="container-fluid px-4">
        ...
        <div class="alert alert-secondary">
            <p class="mb-0">Catat semua utang (uang yang Anda pinjam) dan piutang (uang yang Anda pinjamkan) di sini. Mengelola ini dengan baik akan membantu menjaga hubungan baik dan kesehatan finansial Anda.</p>
        </div>
        ...
        <div class="card mb-4">
            <div class="card-header"><i class="fas fa-plus me-1"></i><?= $form_mode == 'edit' ? 'Edit Catatan: ' . htmlspecialchars($edit_data['person_name']) : 'Tambah Catatan Baru'; ?></div>
            <div class="card-body">
                <form method="POST" action="debts.php">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" class="form-control price-format" name="total_amount_formatted" value="<?= number_format($edit_data['total_amount'] ?? 0, 0, ',', '.'); ?>" required>
                            <input type="hidden" name="total_amount" value="<?= $edit_data['total_amount'] ?? 0; ?>">
                        </div>
                        <div class="col-md-6 mb-3">
        ...
        <div class="card mb-4">
            ...
            <div class="card-body">
                <table id="datatablesSimple" class="table table-striped table-hover">
                    ...
                    <tbody>
                        <?php mysqli_data_seek($resultDebts, 0); while ($debt = mysqli_fetch_assoc($resultDebts)) { ?>
                        <tr>
                            <td>
            ...
            <form method="POST" action="debts.php">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control price-format" name="amount_formatted" required>
