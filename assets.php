...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['action'])) {
    ...
    try {
        if ($_POST['action'] == 'tambah') {
            ...
            // Hitung total modal berdasarkan kuantitas numerik saja
            $quantity_num = floatval(preg_replace('/[^0-9.]/', '', $quantity));
            $total_modal = $quantity_num * $average_buy_price;
            ...
            // current_price diisi dengan total modal saat pertama kali beli
            ...
            $current_quantity_num = floatval(preg_replace('/[^0-9.]/', '', $asset['quantity']));
            $quantity_to_sell_num = floatval(preg_replace('/[^0-9.]/', '', $quantity_to_sell_str));
            ...
            if ($quantity_to_sell_num == $current_quantity_num) {
                ...
                $unit_text = preg_replace('/[0-9.]/', '', $asset['quantity']); // Ekstrak unit (cth: gram, lot)
            ...
            $asset_id = (int)$_POST['asset_id'];
            $new_total_value = (float)str_replace('.', '', $_POST['new_total_value'] ?? 0);
...
foreach($assets_data as $asset) {
    $total_nilai = $asset['current_price'];
...
$page_title = "Alokasi Aset - Uangmu App"; 
$active_page = 'assets';
...
<main>
    <div class="container-fluid px-4">
        ...
        <div class="alert alert-secondary">
            <p class="mb-0">Catat dan pantau semua aset investasi Anda di sini, seperti saham, reksa dana, emas, atau properti. Memahami alokasi aset membantu Anda dalam diversifikasi dan mencapai tujuan keuangan jangka panjang.</p>
        </div>
        ...
        <div class="row">
            <div class="col-lg-5">
                <div class="card mb-4">
                    <div class="card-header"><i class="fas fa-chart-pie me-1"></i>Diagram Alokasi Aset (berdasarkan Nilai Pasar)</div>
                    <div class="card-body"><canvas id="myPieChart" width="100%" height="40"></canvas></div>
                </div>
            </div>
            <div class="col-lg-7">
                <div class="card mb-4">
                    <div class="card-header"><i class="fas fa-plus me-1"></i><?= $form_mode == 'edit' ? 'Edit Aset: ' . htmlspecialchars($edit_data['asset_name']) : 'Beli Aset Baru'; ?></div>
                    <div class="card-body">
                        <form method="POST" action="assets.php<?= $form_mode == 'edit' ? '?action=edit&id='.$edit_data['id'] : '' ?>">
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" name="sumber_dana" required>
                                    ...
                                    <option value="<?= $acc['id']; ?>"><?= htmlspecialchars($acc['account_name']); ?> (Saldo: Rp <?= number_format($acc['current_balance'], 0, ',', '.'); ?>)</option>
                                    <?php endwhile; ?>
                                </select>
                            </div>
                            <?php endif; ?>
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3"><label class="form-label">Nama Aset</label><input type="text" class="form-control" name="asset_name" placeholder="Cth: Emas Antam" value="<?= htmlspecialchars($edit_data['asset_name'] ?? ''); ?>" required></div>
                                <div class="col-md-6 mb-3"><label class="form-label">Tipe Aset</label><input type="text" class="form-control" name="asset_type" placeholder="Cth: Saham, Emas" value="<?= htmlspecialchars($edit_data['asset_type'] ?? ''); ?>" required></div>
                            </div>
                            <div class="row">
                                <div class="col-md-6 mb-3"><label class="form-label">Kuantitas</label><input type="text" class="form-control" name="quantity" placeholder="Cth: 1.5 gram atau 10 lot" value="<?= htmlspecialchars($edit_data['quantity'] ?? ''); ?>" required></div>
                                <div class="col-md-6 mb-3"><label class="form-label">Harga Beli Rata-Rata per Unit</label><input type="text" class="form-control price-format" name="average_buy_price_formatted" value="<?= number_format($edit_data['average_buy_price'] ?? 0, 0, ',', '.'); ?>"><input type="hidden" name="average_buy_price" value="<?= $edit_data['average_buy_price'] ?? 0; ?>"></div>
                            </div>
        ...
        <div class="card mb-4">
            <div class="card-header"><i class="fas fa-table me-1"></i>Portofolio Aset Anda</div>
            <div class="card-body">
                <table id="datatablesSimple" class="table table-striped table-hover">
                    ...
                    <tbody>
                        <?php foreach ($assets_data as $asset) { 
                            $quantity_num = floatval(preg_replace('/[^0-9.]/', '', $asset['quantity']));
                            $total_modal = $asset['average_buy_price'] * $quantity_num; 
                            $total_nilai_pasar = $asset['current_price']; 
                            $profit_loss_rp = $total_nilai_pasar - $total_modal; 
                            $profit_loss_percent = ($total_modal > 0) ? ($profit_loss_rp / $total_modal) * 100 : 0;
                        ...
                        ?>
                        <tr>
                            <td><strong><?= htmlspecialchars($asset['asset_name']); ?></strong><br><small class="text-muted"><?= htmlspecialchars($asset['quantity']); ?> (@ Rp <?= number_format($asset['average_buy_price'],0,',','.') ?>)</small></td>
...
<div class="modal fade" id="adjustPriceModal<?= $asset['id']; ?>" tabindex="-1"><div class="modal-dialog"><div class="modal-content"><div class="modal-header bg-secondary text-white"><h5 class="modal-title">Update Harga Pasar: <?= htmlspecialchars($asset['asset_name']); ?></h5><button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button></div><form method="POST" action="assets.php"><input type="hidden" name="action" value="adjust_value"><input type="hidden" name="asset_id" value="<?= $asset['id']; ?>"><div class="modal-body"><p class="small text-muted">Nilai pasar saat ini: <strong>Rp <?= number_format($asset['current_price'], 0, ',', '.'); ?></strong></p><div class="mb-3"><label class="form-label">Total Nilai Pasar Baru (Rp)</label><input type="text" class="form-control price-format" name="new_total_value_formatted" placeholder="0" required><input type="hidden" name="new_total_value" value="0"></div></div><div class="modal-footer"><button type="button" class="btn btn-light" data-bs-dismiss="modal">Batal</button><button type="submit" class="btn btn-primary">Simpan Perubahan</button></div></form></div></div></div>
