...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['action'])) {
    ...
    try {
        if ($_POST['action'] == 'tambah') {
            ...
            $amount_new = (float)str_replace('.', '', $_POST['amount'] ?? 0);
        ...
        // Alihkan setelah commit berhasil
        if (!headers_sent()) {
...
$queryTransactions = mysqli_prepare($koneksi, "
    SELECT t.*, a.account_name, c.category_name, c.category_icon 
    FROM transactions t 
...
<main>
    <div class="container-fluid px-4">
        ...
        <div class="alert alert-secondary">
            <p class="mb-0">Di sini Anda dapat mencatat semua pemasukan dan pengeluaran. Pencatatan yang disiplin adalah kunci untuk memahami ke mana uang Anda pergi.</p>
        </div>
        ...
        <div class="card mb-4">
            ...
            <div class="card-body">
                <form method="POST" action="transactions.php<?= $form_mode == 'edit' ? '?action=edit&id='.$edit_data['id'] : '' ?>">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4 mb-3">
                            <label class="form-label" id="amountLabel">Jumlah (Rp)</label>
                            <input type="text" class="form-control price-format" name="amount_formatted" value="<?= number_format($edit_data['amount'] ?? 0, 0, ',', '.'); ?>">
                            <input type="hidden" name="amount" value="<?= $edit_data['amount'] ?? 0; ?>">
                        </div>
                    </div>
                    <div class="row">
                ...
                <table id="datatablesSimple" class="table table-striped table-hover align-middle">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <?php if (!empty($trx['category_name'])): ?>
                                    <i class="<?= htmlspecialchars($trx['category_icon']); ?> me-2 text-muted"></i>
                                    <?= htmlspecialchars($trx['category_name']); ?>
                                <?php else: ?>
                                    N/A
                                <?php endif; ?>
                            </td>
                            <td class="text-end fw-bold <?= $trx['transaction_type'] == 'Pengeluaran' ? 'text-danger' : 'text-success'; ?>">
                                Rp <?= number_format($trx['amount'], 2, ',', '.'); ?>
                            </td>
                            <td>
                                <a href="transactions.php?action=edit&id=<?= $trx['id']; ?>" class="btn btn-warning btn-sm" title="Edit"><i class="fas fa-edit"></i></a>
                                <button type="button" class="btn btn-danger btn-sm" data-bs-toggle="modal" data-bs-target="#hapusModal<?= $trx['id']; ?>" title="Hapus"><i class="fas fa-trash"></i></button>
                            </td>
                        </tr>
                        <?php } ?>
                    </tbody>
    ...
    window.addEventListener(\'DOMContentLoaded\', event => {
        ...
        function updateCategoryOptions() {
            ...
            if (selectedType === "Pemasukan" || selectedType === "Pengeluaran") {
                ...
                amountLabel.textContent = "Jumlah Saldo Baru (Rp)";
                categorySelect.required = false;
            }
        }

        transactionTypeRadios.forEach(radio => radio.addEventListener("change", updateCategoryOptions));
