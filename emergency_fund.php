...
$stmt_check = mysqli_prepare($koneksi, "SELECT * FROM emergency_fund WHERE user_id = ?");
mysqli_stmt_bind_param($stmt_check, "i", $user_id);
mysqli_stmt_execute($stmt_check);
$ef_data = mysqli_stmt_get_result($stmt_check);

if (mysqli_num_rows($ef_data) == 0) {
    // FIX: Menggunakan prepared statement untuk insert data awal
    $stmt_insert = mysqli_prepare($koneksi, "INSERT INTO emergency_fund (user_id, target_amount, current_amount) VALUES (?, 0, 0)");
    mysqli_stmt_bind_param($stmt_insert, "i", $user_id);
    mysqli_stmt_execute($stmt_insert);
    
    // Ambil lagi datanya setelah dibuat
    mysqli_stmt_execute($stmt_check); // Eksekusi ulang query select
    $ef_data = mysqli_stmt_get_result($stmt_check);
}
...
<main>
    <div class="container-fluid px-4">
        ...
        <div class="alert alert-secondary">
            <p class="mb-0">Dana darurat adalah jaring pengaman finansial Anda. Siapkan dana untuk kebutuhan tak terduga agar tujuan keuangan jangka panjang Anda tidak terganggu.</p>
        </div>
                ...
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                        <table class="table table-sm table-striped">
                            <tbody>
                                <?php mysqli_data_seek($queryHistory, 0); while($hist = mysqli_fetch_assoc($queryHistory)): ?>
                                <tr>
