...
<main>
    <div class="container-fluid px-4">
        ...
        <div class="alert alert-secondary">
            <p class="mb-0">Punya impian yang ingin diwujudkan? Tetapkan target tabungan Anda di sini, baik itu untuk liburan, gadget baru, atau dana pendidikan. Pantau progres Anda dan tetap termotivasi!</p>
        </div>
        ...
        <div class="card mb-4">
            ...
            <div class="card-body">
                <form method="POST" action="goals.php" enctype="multipart/form-data">
                    ...
                    <div class="row"><div class="col-md-6 mb-3"><label class="form-label">Nama Target</label><input type="text" class="form-control" name="goal_name" value="<?= htmlspecialchars($edit_data['goal_name'] ?? ''); ?>" required></div><div class="col-md-6 mb-3"><label class="form-label">Jumlah Target (Rp)</label><input type="text" class="form-control price-format" name="target_amount_formatted" value="<?= number_format($edit_data['target_amount'] ?? 0, 0, ',', '.'); ?>" required><input type="hidden" name="target_amount" value="<?= $edit_data['target_amount'] ?? 0; ?>"></div></div>
...
<script>
    window.addEventListener(\'DOMContentLoaded\', event => {
        function formatRupiah(angkaStr) { let number_string = String(angkaStr).replace(/[^0-9]/g, \'\'); return number_string.replace(/\B(?=(\d{3})+(?!\d))/g, "."); }
        function unformatRupiah(str) { return String(str).replace(/\./g, \'\'); }
