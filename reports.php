...
// 3. Data untuk Pie Chart & Tabel Ringkasan Kategori
$stmt_categories = mysqli_prepare($koneksi, "
    SELECT c.category_name, c.category_icon, SUM(t.amount) as total 
    FROM transactions t 
    JOIN categories c ON t.category_id = c.id 
    WHERE t.user_id = ? AND t.transaction_type = 'Pengeluaran' AND t.transaction_date BETWEEN ? AND ? 
...
$page_title = "Laporan Keuangan - Uangmu App"; 
$active_page = 'reports';
...
<main>
    <div class="container-fluid px-4">
        ...
        <div class="alert alert-secondary">
            <p class="mb-0">Analisis kesehatan finansial Anda melalui laporan ini. Gunakan filter tanggal untuk melihat ringkasan keuangan pada periode tertentu dan ekspor data Anda untuk analisis lebih lanjut.</p>
        </div>
        ...
        <div class="row">
            ...
            <div class="col-xl-4 col-md-12"><div class="card <?= $selisih >= 0 ? 'bg-primary' : 'bg-warning text-dark'; ?> text-white mb-4"><div class="card-body"><h5><?= $selisih >= 0 ? 'Surplus' : 'Defisit'; ?></h5><h4 class="display-6">Rp <?= number_format($selisih, 0, ',', '.'); ?></h4></div></div></div>
        </div>
            ...
            <div class="col-lg-7">
                <div class="card mb-4"><div class="card-header"><i class="fas fa-list-alt me-1"></i>Ringkasan Pengeluaran per Kategori</div><div class="card-body">
                    <table class="table table-striped table-hover table-sm align-middle">
                        ...
                        <tbody>
                            <?php if(mysqli_num_rows($category_summary) > 0): ?>
                                <?php mysqli_data_seek($category_summary, 0); while($cat = mysqli_fetch_assoc($category_summary)): 
                                    $chart_labels[] = $cat['category_name'];
                                    $chart_data[] = $cat['total'];
                                ?>
                                <tr>
                                    <td>
                                        <i class="<?= htmlspecialchars($cat['category_icon']); ?> me-2 text-muted"></i>
                                        <?= htmlspecialchars($cat['category_name']); ?>
                                    </td>
                                    <td class="text-end">Rp <?= number_format($cat['total'], 0, ',', '.'); ?></td>
                                </tr>
                                <?php endwhile; ?>
                            <?php else: ?>
                                <tr><td colspan="2" class="text-center">Tidak ada data pengeluaran pada periode ini.</td></tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
                </div></div>
